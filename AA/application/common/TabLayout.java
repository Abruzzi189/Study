 ...
 * notified when any tab's selection state has been changed. <p> If you're using a {@link
 * android.support.v4.view.ViewPager} together with this layout, you can use {@link
 * #addTabsFromPagerAdapter(PagerAdapter)} which will populate the tabs using the {@link
 * PagerAdapter}'s page titles. You should also use a {@link ViewPager.OnPageChangeListener} to
 * forward the scroll and selection changes to this layout. You can use the one returned {@link
 * #createOnPageChangeListener()} for easy implementation:
 *
 * <pre>
 * ViewPager viewPager = ...;
 * TabLayout tabLayout = ...;
...
public class TabLayout extends HorizontalScrollView {
  ...
  public TabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    if (a.hasValue(R.styleable.TabLayout_tabSelectedTextColor)) {
      mTabSelectedTextColor = a.getColor(
          R.styleable.TabLayout_tabSelectedTextColor, 0);
      mTabSelectedTextColorSet = true;
    }

    mTabMinWidth = a.getDimensionPixelSize(
        R.styleable.TabLayout_tabMinWidth, 0);
    mRequestedTabMaxWidth = a.getDimensionPixelSize(
        R.styleable.TabLayout_tabMaxWidth, 0);

    mTabBackgroundResId = a.getResourceId(
        R.styleable.TabLayout_tabBackground, 0);

    mContentInsetStart = a.getDimensionPixelSize(
        R.styleable.TabLayout_tabContentStart, 0);
   ...
   * part of a scrolling container such as {@link ViewPager}. <p> Calling this method does not
   * update the selected tab, it is only used for drawing purposes.
   */
  ...
  /**
   * Add new {@link Tab}s populated from a {@link PagerAdapter}. Each tab will have it's text set to
   * the value returned from {@link PagerAdapter#getPageTitle(int)}.
   *
  ...
  /**
   * Create a {@link ViewPager.OnPageChangeListener} which implements the necessary calls back to
   * this layout so that the tabs position is kept in sync. <p> If you need to have a custom {@link
   * ViewPager.OnPageChangeListener} for your own purposes, you can still use the instance returned
   * from this method, but making sure to call through to all of the methods.
   */
  ...
  /**
   * Set the {@link android.support.design.widget.TabLayout.OnTabSelectedListener} that will handle
   * switching to and from tabs.
   *
  ...
  /**
   * Set the behavior mode for the Tabs in this layout. The valid input options are: <ul> <li>{@link
   * #MODE_FIXED}: Fixed tabs display all tabs concurrently and are best used with content that
   * benefits from quick pivots between tabs.</li> <li>{@link #MODE_SCROLLABLE}: Scrollable tabs
   * display a subset of tabs at any given moment, and can contain longer tab labels and a larger
   * number of tabs. They are best used for browsing contexts in touch interfaces when users don’t
   * need to directly compare the tab labels. This mode is commonly used with a {@link
   * ViewPager}.</li> </ul>
   *
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    // If we have a MeasureSpec which allows us to decide our height, try
    // and use the default
    // height
    switch (MeasureSpec.getMode(heightMeasureSpec)) {
      case MeasureSpec.AT_MOST:
        heightMeasureSpec = MeasureSpec.makeMeasureSpec(
            Math.min(dpToPx(DEFAULT_HEIGHT),
                MeasureSpec.getSize(heightMeasureSpec)),
            MeasureSpec.EXACTLY);
        break;
      case MeasureSpec.UNSPECIFIED:
        heightMeasureSpec = MeasureSpec.makeMeasureSpec(
            dpToPx(DEFAULT_HEIGHT), MeasureSpec.EXACTLY);
        break;
    }
    ...
    if (mMode == MODE_FIXED && getChildCount() == 1) {
      // If we're in fixed mode then we need to make the tab strip is the
      // same width as us
      // so we don't scroll
      ...
      if (child.getMeasuredWidth() > width) {
        // If the child is wider than us, re-measure it with a widthSpec
        // set to exact our
        // measure width
    ...
    // Now update the tab max width. We do it here as the default tab min
    // width is
    // layout width - 56dp
    int maxTabWidth = mRequestedTabMaxWidth;
    final int defaultTabMaxWidth = getMeasuredWidth()
        - dpToPx(TAB_MIN_WIDTH_MARGIN);
    if (maxTabWidth == 0 || maxTabWidth > defaultTabMaxWidth) {
      // If the request tab max width is 0, or larger than our default,
      // use the default
    ...
    if (startScrollX != targetScrollX) {
      final Animation animation = new Animation() {
        ...
        protected void applyTransformation(float interpolatedTime,
            Transformation t) {
          final float value = lerp(startScrollX, targetScrollX,
              interpolatedTime);
  ...
  void changeTab(Tab tab) {
    if (mSelectedTab == tab) {
      ...
      final int newPosition = tab != null ? tab.getPosition()
  ...
  private int calculateScrollXForTab(int position, float positionOffset) {
    if (mMode == MODE_SCROLLABLE) {
      ...
      final View nextChild = position + 1 < mTabStrip.getChildCount() ? mTabStrip
          .getChildAt(position + 1) : null;
      final int selectedWidth = selectedChild != null ? selectedChild
          .getWidth() : 0;
      final int nextWidth = nextChild != null ? nextChild.getWidth() : 0;

      return (int) (selectedChild.getLeft()
          + ((selectedWidth + nextWidth) * positionOffset * 0.5f)
          + selectedChild.getWidth() * 0.5f - getWidth() * 0.5f);
    }
    return 0;
  }
  ...
  private void updateTabViewsLayoutParams() {
    for (int i = 0; i < mTabStrip.getChildCount(); i++) {
      ...
      updateTabViewLayoutParams((LinearLayout.LayoutParams) child
          .getLayoutParams());
      child.requestLayout();
    }
  }
  ...
  public static final class Tab {
    ...
    public Tab setCustomView(int layoutResId) {
      return setCustomView(LayoutInflater.from(mParent.getContext())
    ...
    public Tab setIcon(int resId) {
      return setIcon(ContextCompat.getDrawable(mParent.getContext(),
          resId));
    }
  ...
  class TabView extends LinearLayout implements OnLongClickListener {
    ...
    public TabView(Context context, Tab tab) {
      ...
      if (sdk < android.os.Build.VERSION_CODES.JELLY_BEAN) {
        ...
        setBackground(getResources().getDrawable(mTabBackgroundResId));
      }
      ViewCompat.setPaddingRelative(this, mTabPaddingStart,
          mTabPaddingTop, mTabPaddingEnd, mTabPaddingBottom);
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
      ...
      if (mTabMaxWidth != 0 && getMeasuredWidth() > mTabMaxWidth) {
        ...
        super.onMeasure(MeasureSpec.makeMeasureSpec(mTabMaxWidth,
            MeasureSpec.EXACTLY), heightMeasureSpec);
      } else if (mTabMinWidth > 0 && getMeasuredHeight() < mTabMinWidth) {
        // Re-measure if we're below our minimum size.
        super.onMeasure(MeasureSpec.makeMeasureSpec(mTabMinWidth,
            MeasureSpec.EXACTLY), heightMeasureSpec);
      }
    }
      ...
      if (custom != null) {
        ...
        if (hasText) {
          if (mTextView == null) {
            ...
            addView(textView, LayoutParams.WRAP_CONTENT,
                LayoutParams.WRAP_CONTENT);
            mTextView = textView;
          }
    ...
    public boolean onLongClick(View v) {
      ...
      Toast cheatSheet = Toast.makeText(context,
          mTab.getContentDescription(), Toast.LENGTH_SHORT);
      // Show under the tab
    ...
    private ColorStateList createColorStateList(int defaultColor,
        int selectedColor) {
  ...
  private class SlidingTabStrip extends LinearLayout {
    ...
    protected void onMeasure(final int widthMeasureSpec,
      ...
      if (MeasureSpec.getMode(widthMeasureSpec) != MeasureSpec.EXACTLY) {
        // HorizontalScrollView will first measure use with UNSPECIFIED,
        // and then with
        // EXACTLY. Ignore the first call since anything we do will be
        // overwritten anyway
        return;
      }
      ...
      if (mMode == MODE_FIXED && mTabGravity == GRAVITY_CENTER) {
        ...
        final int unspecifiedSpec = MeasureSpec.makeMeasureSpec(0,
        ...
        for (int i = 0, z = count; i < z; i++) {
          ...
          largestTabWidth = Math.max(largestTabWidth,
              child.getMeasuredWidth());
        }

        if (largestTabWidth <= 0) {
          // If we don't have a largest child yet, skip until the next
          // measure pass
          return;
        }
        ...
        if (largestTabWidth * count <= getMeasuredWidth() - gutter * 2) {
          // If the tabs fit within our width minus gutters, we will
          // set all tabs to have
          // the same width
          ...
          // If the tabs will wrap to be larger than the width minus
          // gutters, we need
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
      ...
      if (!isAnimationRunning(getAnimation())) {
        // If we've been layed out, and we're not currently in an
        ...
        final Animation anim = new Animation() {
          ...
          protected void applyTransformation(float interpolatedTime,
            ...
            setIndicatorPosition(
                (int) lerp(startLeft, targetLeft,
                    interpolatedTime),
                (int) lerp(startRight, targetRight,
                    interpolatedTime));
          }
        };

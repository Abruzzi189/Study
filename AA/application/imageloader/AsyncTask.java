 ...
 *
 * There are 3 changes in this copy of AsyncTask: -pre-HC a single thread executor is used for
 ...
 *
 * <h2>The 4 steps</h2> <p>When an asynchronous task is executed, the task goes through 4 steps:</p>
 * <ol> <li>{@link #onPreExecute()}, invoked on the UI thread immediately after the task is
 * executed. This step is normally used to setup the task, for instance by showing a progress bar in
 * the user interface.</li> <li>{@link #doInBackground}, invoked on the background thread
 * immediately after {@link #onPreExecute()} finishes executing. This step is used to perform
 * background computation that can take a long time. The parameters of the asynchronous task are
 * passed to this step. The result of the computation must be returned by this step and will be
 * passed back to the last step. This step can also use {@link #publishProgress} to publish one or
 * more units of progress. These values are published on the UI thread, in the {@link
 * #onProgressUpdate} step.</li> <li>{@link #onProgressUpdate}, invoked on the UI thread after a
 * call to {@link #publishProgress}. The timing of the execution is undefined. This method is used
 * to display any form of progress in the user interface while the background computation is still
 * executing. For instance, it can be used to animate a progress bar or show logs in a text
 * field.</li> <li>{@link #onPostExecute}, invoked on the UI thread after the background computation
 * finishes. The result of the background computation is passed to this step as a parameter.</li>
 * </ol>
 *
 ...
 * automatically as of {@link android.os.Build.VERSION_CODES#JELLY_BEAN}.</li> <li>The task instance
 ...
 * #doInBackground}, {@link #onProgressUpdate} manually.</li> <li>The task can be executed only once
 * (an exception will be thrown if a second execution is attempted.)</li> </ul>
 *
 * <h2>Memory observability</h2> <p>AsyncTask guarantees that all callback calls are synchronized in
 ...
 * application errors caused by parallel execution.</p> <p>If you truly want parallel execution, you
...
public abstract class AsyncTask<Params, Progress, Result> {
    ...
    mFuture = new FutureTask<Result>(mWorker) {
      ...
      protected void done() {
        try {
          ...
          LogUtils.w(LOG_TAG, e.getMessage());
  ...
  /**
   * <p>Applications should preferably override {@link #onCancelled(Object)}. This method is invoked
  ...
  /**
   * <p>Attempts to cancel execution of this task.  This attempt will fail if the task has already
   ...
   * {@link android.os.Build.VERSION_CODES#HONEYCOMB}, tasks are back to being executed on a single
   ...
   * leading to obscure data loss and stability issues.  Such changes are best executed in serial;
   * to guarantee such work is serialized regardless of platform version you can use this function

...
public class ContactImageCache {
   ...
   */
  private ContactImageCache(float memCacheSizePercent) {
    init(memCacheSizePercent);
  }

  /**
   * Find and return an existing ImageCache stored in a {@link RetainFragmentContact}, if not found
   * a new one is created using the supplied params and saved to a {@link RetainFragmentContact}.
   *
  ...
  public static ContactImageCache getInstance(
    ...
    final RetainFragmentContact mRetainFragment = findOrCreateRetainFragment(fragmentManager);

    // See if we already have an ImageCache stored in RetainFragment
    ContactImageCache imageCache = (ContactImageCache) mRetainFragment
        .getObject();

    // No existing ImageCache, create one and store it in RetainFragment
    if (imageCache == null) {
      imageCache = new ContactImageCache(memCacheSizePercent);
   ...
   * setting percent to 0.2 would set the memory cache to one fifth of the available memory. Throws
   ...
   */
  public static RetainFragmentContact findOrCreateRetainFragment(
      FragmentManager fm) {
    // Check to see if we have retained the worker fragment.
    RetainFragmentContact mRetainFragment = (RetainFragmentContact) fm
   ...
   */
  public static class RetainFragmentContact extends Fragment {

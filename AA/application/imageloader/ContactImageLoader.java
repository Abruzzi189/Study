 ...
 */
public abstract class ContactImageLoader {
  ...
  protected ContactImageLoader(Context context, int imageSize) {
   ...
   * the closest inSampleSize that will result in the final decoded bitmap having a width and height
   * equal to or larger than the requested width and height. This implementation does not ensure a
   * power of 2 is returned for inSampleSize which can be faster when decoding but results in a
   ...
   * ContactImageLoader#processBitmap(Object)} to define the processing logic). If the image is
  ...
  /**
   * Adds an {@link ContactImageCache} to this image loader.
   *
   * @param fragmentManager A FragmentManager to use to retain the cache over configuration changes
  ...
  public void addImageCache(FragmentManager fragmentManager,
    ...
    mImageCache = ContactImageCache.getInstance(fragmentManager,
        memCacheSizePercent);
  }
   ...
   * ContactImageLoader#loadImage(Object, ImageView)}
  ...
  private class BitmapWorkerTask extends AsyncTask<Object, Void, Bitmap> {
    ...
    protected Bitmap doInBackground(Object... params) {
      ...
      // If the task has not been cancelled by another thread and the
      // ImageView that was
      // originally bound to this task is still bound back to this task
      // and our "exit early"
      // flag is not set, then call the main process method (as

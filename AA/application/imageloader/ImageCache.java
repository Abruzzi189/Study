...
/**
 * This class handles disk and memory caching of bitmaps in conjunction with the {@link ImageWorker}
 * class and its subclasses. Use {@link ImageCache#getInstance(FragmentManager, ImageCacheParams)}
 * to get an instance of this class, although usually a cache should be added directly to an {@link
 * ImageWorker} by calling {@link ImageWorker#addImageCache(FragmentManager, ImageCacheParams)}.
 */
public class ImageCache {
  ...
  /**
   * Create a new ImageCache object using the specified parameters. This should not be called
   * directly by other classes, instead use {@link ImageCache#getInstance(FragmentManager,
   * ImageCacheParams)} to fetch an ImageCache instance.
   *
  ...
  private void init(ImageCacheParams cacheParams) {
    ...
    if (mCacheParams.memoryCacheEnabled) {
      if (BuildConfig.DEBUG) {
        LogUtils.d(TAG, "Memory cache created (size = "
   ...
   */
  public Bitmap getBitmapFromDiskCache(String data, int width, int height) {
    final String key = hashKeyForDisk(data);
    Bitmap bitmap = null;
    ...
    synchronized (mDiskCacheLock) {
      ...
      if (mDiskLruCache != null) {
        ...
        try {
          ...
          LogUtils.e(TAG, "getBitmapFromDiskCache - " + e);
        } finally {
          try {
            if (inputStream != null) {
              inputStream.close();
   ...
   */
  protected synchronized Bitmap getBitmapFromReusableSet(
  ...
  public static class ImageCacheParams {
    ...
    /**
     * Create a set of image cache parameters that can be provided to {@link
     * ImageCache#getInstance(FragmentManager, ImageCacheParams)} or {@link
     * ImageWorker#addImageCache(FragmentManager, ImageCacheParams)}.
     *
     ...
     * percent to 0.2 would set the memory cache to one fifth of the available memory. Throws {@link
     * IllegalArgumentException} if percent is < 0.05 or > .8. memCacheSize is stored in kilobytes
     * instead of bytes as this will eventually be passed to construct a LruCache which takes an int
     * in its constructor.
     *
     * This value should be chosen carefully based on a number of factors Refer to the corresponding
     * Android Training class for more discussion: http://developer.android.com/training/displaying-bitmaps/
     *

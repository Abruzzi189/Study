...
/**
 * Wraps the Cursor returned by an ordinary FileProvider, StreamProvider, or other ContentProvider.
 * If the query() requests _DATA or MIME_TYPE, adds in some values for that column, so the client
 * getting this Cursor is less likely to crash. Of course, clients should not be requesting either
 * of these columns in the first place...
 */
public class LegacyCompatCursorWrapper extends CursorWrapper {
   ...
   *
   * @param cursor the Cursor to be wrapped
   * @param mimeType the MIME type of the content represented by the Uri that generated this Cursor,
   * should we need it
   */
  ...
  /**
   * @return true if the Cursor has a MIME_TYPE column, false otherwise
   */

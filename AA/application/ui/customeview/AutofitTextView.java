...
public class AutofitTextView extends TextView {
  ...
  private static float getTextSize(String text, TextPaint paint,
    ...
    if (lineCount > maxLines) {
      return getTextSize(text, paint, targetWidth, maxLines, low, mid,
          precision, displayMetrics);
    } else if (lineCount < maxLines) {
      return getTextSize(text, paint, targetWidth, maxLines, mid, high,
          precision, displayMetrics);
    } else {
      ...
      if ((high - low) < precision) {
        ...
        return getTextSize(text, paint, targetWidth, maxLines, low,
            mid, precision, displayMetrics);
      } else if (maxLineWidth < targetWidth) {
        return getTextSize(text, paint, targetWidth, maxLines, mid,
  ...
  private static int getLineCount(String text, TextPaint paint, float size,
      float width, DisplayMetrics displayMetrics) {
    paint.setTextSize(TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX,
        size, displayMetrics));
  ...
  /**
   * Set the minimum text size to the given value, interpreted as "scaled pixel" units. This size is
   * adjusted based on the current density and user font size preference.
   *
  ...
  /**
   * Set the minimum text size to a given unit and value. See TypedValue for the possible dimension
   * units.
   *
  ...
  public void setMinTextSize(int unit, float minSize) {
    ...
    setRawMinTextSize(TypedValue.applyDimension(unit, minSize,
        r.getDisplayMetrics()));
  }
  ...
  /**
   * Set the amount of precision used to calculate the correct text size to fit within it's bounds.
  ...
  private void refitText() {
    ...
    if (targetWidth > 0) {
      ...
      if ((mMaxLines == 1 && mPaint.measureText(text) > targetWidth)
          || getLineCount(text, mPaint, size, targetWidth,
          displayMetrics) > mMaxLines) {
        size = getTextSize(text, mPaint, targetWidth, mMaxLines, low,
            high, mPrecision, displayMetrics);
      }

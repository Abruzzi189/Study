...
import com.application.ui.customeview.pulltorefresh.PullToRefreshBase.OnPullEventListener;
import com.application.ui.customeview.pulltorefresh.PullToRefreshBase.OnRefreshListener;
import com.application.ui.customeview.pulltorefresh.PullToRefreshBase.OnRefreshListener2;
import com.application.ui.customeview.pulltorefresh.PullToRefreshBase.State;

public interface IPullToRefresh<T extends View> {

  /**
   * Demos the Pull-to-Refresh functionality to the user so that they are aware it is there. This
   * could be useful when the user first opens your app, etc. The animation will only happen if the
   * Refresh View (ListView, ScrollView, etc) is in a state where a Pull-to-Refresh could occur by a
   ...
   * only use touch events where the difference in the Y-axis is greater than the difference in the
   * X-axis. This means that the View will not interfere when it is used in a horizontal scrolling
   * View (such as a ViewPager).
   *
   ...
   * means that the View will not interfere when it is used in a horizontal scrolling View (such as
   * a ViewPager), but will restrict which types of finger scrolls will trigger the View.
   *
  ...
  /**
   * Returns a proxy object which allows you to call methods on all of the LoadingLayouts (the Views
   * which show when Pulling/Refreshing). <p /> You should not keep the result of this method any
  ...
  /**
   * Returns a proxy object which allows you to call methods on the LoadingLayouts (the Views which
   * show when Pulling/Refreshing). The actual LoadingLayout(s) which will be affected, are chosen
   * by the parameters you give. <p /> You should not keep the result of this method any longer than
   * you need it.
   *
  ...
  /**
   * Get the Wrapped Refreshable View. Anything returned here has already been added to the content
   * view.
   *
  ...
  /**
   * Gets whether Overscroll support is enabled. This is different to Android's standard Overscroll
   * support (the edge-glow) which is available from GINGERBREAD onwards
   *
  ...
  /**
   * Sets whether Overscroll support is enabled. This is different to Android's standard Overscroll
   * support (the edge-glow). This setting only takes effect when running on device with Android
   * v2.3 or greater.
   *
  ...
  /**
   * By default the Widget disables scrolling on the Refreshable View while refreshing. This method
   ...
   *
   * @param listener - Listener to be used when the Widget has a pull event to propogate.
   */

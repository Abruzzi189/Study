...
import com.application.ui.customeview.pulltorefresh.PullToRefreshBase.Orientation;
...
public abstract class LoadingLayout extends FrameLayout implements
  ...
  private CharSequence mManualReleaseLabel;
  private CharSequence mLastUpdate;
  ...
  public LoadingLayout(Context context, final Mode mode,
    ...
    mOutterLayout = (FrameLayout) findViewById(R.id.fl_outter);
    mInnerLayout = (RelativeLayout) findViewById(R.id.fl_inner);
    mHeaderText = (TextView) mInnerLayout
        .findViewById(R.id.pull_to_refresh_text);
    mHeaderText.setTextColor(getResources().getColor(
    ...
    mSubHeaderText.setTextColor(getResources().getColor(
        R.color.color_hint_bold));
    mHeaderImage = (ImageView) mInnerLayout
        .findViewById(R.id.pull_to_refresh_image);
    mManualReleaseLabel = context
        .getString(R.string.pull_to_refresh_manual_refreshing_label);

    FrameLayout.LayoutParams lp = (FrameLayout.LayoutParams) mOutterLayout
        .getLayoutParams();
  ...
  public final int getContentSize() {
    switch (mScrollDirection) {
      case HORIZONTAL:
        return mOutterLayout.getWidth();
      case VERTICAL:
      default:
        return mOutterLayout.getHeight();
    }
  }
  ...
  public final void pullToRefresh() {
    ...
    if (null != mSubHeaderText) {
      mSubHeaderText.setText(mLastUpdate);
    }

    // Now call the callback
    pullToRefreshImpl();
    mOutterLayout.setVisibility(VISIBLE);
  }
  ...
  public final void refreshing() {
    ...
    String format = getResources().getString(
        R.string.pull_to_refresh_time_update);
    String formatTime = getResources().getString(R.string.pull_dateformat);
    DateFormat dateFormat = new SimpleDateFormat(formatTime,
        Locale.getDefault());
    Date date = new Date();
    String timeUpdate = dateFormat.format(date);
    mLastUpdate = String.format(format, timeUpdate);
  ...
  public final void manualRefresh() {
    if (null != mHeaderText) {
      ...
      mHeaderText.setText(mManualReleaseLabel);
    }
    ...
    if (mUseIntrinsicAnimation) {
      ...
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
        Runnable runnable = new Runnable() {
          @Override
          public void run() {
            mHeaderImage.animate().rotationBy(360)
                .withEndAction(this).setDuration(500)
                .setInterpolator(new LinearInterpolator())
                .start();
          }
        };
        mHeaderImage.animate().rotationBy(360).withEndAction(runnable)
            .setDuration(500)
            .setInterpolator(new LinearInterpolator()).start();
      } else {
        mHeaderImage.animate().rotationBy(200 * 360)
            .setDuration(20 * 5000)
            .setInterpolator(new LinearInterpolator()).start();
      }
    }
  ...
  public final void setLoadingDrawable(Drawable imageDrawable) {
    ...
    mOutterLayout.setVisibility(GONE);
    // Now call the callback

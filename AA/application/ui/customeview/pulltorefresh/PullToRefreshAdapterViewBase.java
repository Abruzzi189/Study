...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView>
  ...
  /**
   * Pass-through method for {@link PullToRefreshBase#getRefreshableView() getRefreshableView()}.
   * {@link AdapterView#setOnItemClickListener(OnItemClickListener) setOnItemClickListener(listener)}.
  ...
  private boolean isLastItemVisible() {
    final Adapter adapter = mRefreshableView.getAdapter();
    if ((null == adapter || adapter.isEmpty()) && mRefreshableView.getChildCount() <= 0) {
      return true;
    } else {

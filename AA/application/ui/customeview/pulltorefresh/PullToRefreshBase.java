...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
  ...
  private LoadingLayout mFooterManualLayout;
  ...
  public final void onRefreshComplete() {
    if (isRefreshing()) {
      mFooterManualLayout.setVisibility(View.GONE);
  ...
  public final boolean onTouchEvent(MotionEvent event) {
    ...
    if (isRefreshing()) {
      if (!mScrollingWhileRefreshingEnabled) {
        return true;
      } else if (mState == State.MANUAL_REFRESHING) {
        return false;
      }
    }
  ...
  /**
   * @deprecated You should now call this method on the result of {@link #getLoadingLayoutProxy(boolean,
   * boolean)}.
   */
  ...
  public void setPullLabelHeader(CharSequence pullLabel) {
    mHeaderLayout.setPullLabel(pullLabel);
  }

  public void setReleaseLabelHeader(CharSequence releaseLabel) {
    mHeaderLayout.setReleaseLabel(releaseLabel);
  }

  public void setPullLabelFooter(CharSequence pullLabel) {
    mFooterLayout.setPullLabel(pullLabel);
  }

  public void setReleaseLabelFooter(CharSequence releaseLabel) {
  ...
  /**
   * @return Either {@link Orientation#VERTICAL} or {@link Orientation#HORIZONTAL} depending on the
   * scroll direction.
   */
  ...
  /**
   * This is implemented by derived classes to return the created View. If you need to use a custom
   * View (such as a custom ListView), override this method and return an instance of your custom
   * class.
   * <p/>
   * Be sure to set the ID of the view in this method, especially if you're using a ListActivity or
   * ListFragment.
   *
  ...
  protected void onManualRefreshing() {
    setScrollingWhileRefreshingEnabled(true);
    if (mRefreshableView != null) {
      mFooterManualLayout.setVisibility(View.VISIBLE);
      mFooterManualLayout.manualRefresh();
      if (mRefreshableView instanceof ListView) {
        ListView refreshView = (ListView) mRefreshableView;
        if (refreshView.getFooterViewsCount() < 1) {
          refreshView.addFooterView(mFooterManualLayout);
        }
      } else if (mRefreshableView instanceof GridViewWithHeaderAndFooter) {
        GridViewWithHeaderAndFooter refreshView = (GridViewWithHeaderAndFooter) mRefreshableView;
        if (refreshView.getFooterViewsCount() < 1) {
          refreshView.addFooterView(mFooterManualLayout);
        }
      }
  ...
  protected final void onSizeChanged(int w, int h, int oldw, int oldh) {
    if (DEBUG) {
      LogUtils.d(LOG_TAG,
          String.format("onSizeChanged. W: %d, H: %d", w, h));
    }
  ...
  protected final void setHeaderScroll(int value) {
    ...
    if (mLayoutVisibilityChangesEnabled) {
      if (value < 0) {
        ...
        mFooterManualLayout.setVisibility(View.INVISIBLE);
      }
    }
  ...
  /**
   * Smooth Scroll to position using the longer default duration of {@value
   * #SMOOTH_SCROLL_LONG_DURATION_MS} ms.
   *
   * @param scrollValue - Position to scroll to
   */
  ...
  private void init(Context context, AttributeSet attrs) {
    ...
    mFooterManualLayout = createLoadingLayout(context, Mode.PULL_FROM_END,
        a);

    mFooterManualLayout.setVisibility(View.GONE);
    if (mRefreshableView instanceof GridViewWithHeaderAndFooter) {
      GridViewWithHeaderAndFooter refreshView = (GridViewWithHeaderAndFooter) mRefreshableView;
      if (refreshView.getFooterViewsCount() < 1) {
        refreshView.addFooterView(mFooterManualLayout);
      }
  ...
  public static enum AnimationStyle {
    ...
    /**
     * This is the old default, and what is commonly used on iOS. Uses an arrow image which flips
  ...
  public static enum Mode {
    ...
    /**
     * Only allow the user to Pull from the start of the Refreshable View to refresh. The start is
     * either the Top or Left, depending on the scrolling direction.
     */
    PULL_FROM_START(0x1),
  ...
  public static enum State {
    ...
    /**
     * When the UI is in a state which means that user is not interacting with the Pull-to-Refresh
     * function.
     */
    RESET(0x0),
    ...
    /**
     * When the UI is being pulled by the user, and <strong>has</strong> been pulled far enough so
     * that it will refresh when released.
     */
    ...
    /**
     * When the UI is currently refreshing, caused by a call to {@link
     * PullToRefreshBase#setRefreshing() setRefreshing()}.
     */
   ...
   * Useful when you want to append extra UI events (such as sounds). See ( {@link
   * PullToRefreshAdapterViewBase#setOnPullEventListener}.
   *
   * @author Chris Banes
   */
  public static interface OnPullEventListener<V extends View> {
     ...
     * depending on which direction the user is pulling. Only useful when <var>state</var> is {@link
     * State#PULL_TO_REFRESH} or {@link State#RELEASE_TO_REFRESH}.
     */
